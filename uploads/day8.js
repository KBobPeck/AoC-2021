...
befcg edb bgadfc dfaecb cdfegab egda agcdbe ed dcabg debgc | geda aegd gcdafb dcefab`.split(
    "\n"
  );

const left = input.map((each) => each.split(" | ")[0]);
const right = input.map((each) => each.split(" | ")[1]);

module.exports = { input, left, right };
